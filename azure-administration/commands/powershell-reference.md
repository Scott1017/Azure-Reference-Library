...
| `New-AzRoleAssignment` | Assign an RBAC role to a user | `New-AzRoleAssignment -SignInName "scott@domain.com" -RoleDefinitionName "Contributor" -Scope "/subscriptions/<sub-id>"` | Role assignment creates access control through three core categories. The Owner has full access similar to an Internal Quality Assurer (IQA), the Contributor can create and manage resources similar to a lecturer, and the Reader can only view resources similar to an External Quality Assurer (EQA). Access can be permanently assigned or temporarily granted through Privileged Identity Management (PIM), similar to giving the EQA temporary access to the IQA room while the IQA retains permanent authority. |
